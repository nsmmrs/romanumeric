let home =
  ...
  Sys.getenv_opt env_var |> Option.to_result ~none:(Romanum_error.missing_env env_var)

let default_cache_dir =
  Result.map (fun home ->
      "romanum" |> Filename.concat ".cache" |> Filename.concat home) home

let default_config_dir =
  Result.map (fun home ->
      "romanum" |> Filename.concat ".config" |> Filename.concat home) home
